...
// Compare classic Dijkstra vs bidirectional Dijkstra (kBiDijkstra) for
// consistency.
static void run_bidijkstra_consistency(ways const& w,
  ...
  fmt::println(
      "dijkstra vs bidir-dijkstra congruent (non-empty): {}/{} ({:3.1f}%)",
      non_empty_congruent, non_empty_samples,
      non_empty_samples == 0
          ? 0.0
          : (static_cast<double>(non_empty_congruent) / non_empty_samples) *
                100.0);
  if (non_empty_congruent == non_empty_samples && !baseline_times.empty()) {
    fmt::println(
        "avg speed ratio (baseline/bidir): {:.2f}",
        static_cast<double>(
            std::reduce(begin(baseline_times), end(baseline_times)).count()) /
            static_cast<double>(
                std::reduce(begin(bidir_times), end(bidir_times)).count()));
  }
}
