...
constexpr auto kUseFixedSeed = true;
constexpr auto kForcePreprocessing = true;
constexpr auto kUseMultithreading = true;
constexpr auto kPrintDebugGeojson = false;
constexpr auto kMaxMatchDistance = 100;
constexpr auto kMaxAllowedPathDifferenceRatio = 0.5;
constexpr auto kMaxDetourCost = 10 * 60U;
constexpr auto kOrderingMethod = node_order::method::kRealImportance;

void extract_ch(std::string_view const raw_data,
  ...
  if (!fs::exists(data_dir)) {
    if (fs::exists(raw_data)) {
      ...
      extract(false, raw_data, data_dir, fs::path{});
    }
  }
}

template <bool UseFixedSeed, bool Force>
void preprocess(std::string_view const data_dir) {
  if (auto const f_name =
          shortcut_storage<car::node>::get_filename(search_profile::kCar);
      Force || !fs::exists(fs::path{data_dir} / f_name)) {
    fmt::println("{} not found or FORCED\nContracting...", f_name);
    auto const seed =
        UseFixedSeed
            ? 1337
            : std::chrono::steady_clock::now().time_since_epoch().count();
    preprocess_ch<car>(data_dir, search_profile::kCar, car::parameters{}, seed,
                       kMaxDetourCost, kOrderingMethod);
  } else {
    fmt::println("{} found\nStarting tests...", f_name);
  }
  get_shortcut_storage<car::node>() =
      shortcut_storage<car::node>::read(data_dir, search_profile::kCar);
}

void run_ch(ways const& w,
            lookup const& l,
  ...
  auto const from_tos = [&]() {
    auto prng = std::mt19937{};  // NOLINT(*-msc51-cpp)
    auto distr =
        std::uniform_int_distribution<std::uint32_t>{0, w.n_nodes() - 1};
    auto from_tos_ = std::vector<std::pair<node_idx_t, node_idx_t>>{};
    for (auto i = 0U; i != n_samples; ++i) {
      from_tos_.emplace_back(distr(prng), distr(prng));
    }
    return from_tos_;
  }();
  ...
  auto const single_run = [&](std::pair<node_idx_t, node_idx_t> const from_to) {
    ...
    auto const experiment =
        route(car::parameters{}, w, l, search_profile::kCar, from_loc, to_loc,
              from_matches_span, to_matches_span, max_cost, dir, nullptr,
              nullptr, nullptr, routing_algorithm::kCHDijkstra);
  ...
  if (kUseMultithreading) {
    ...
    std::ranges::for_each(from_tos, single_run);
  }
...
TEST(dijkstra_ch, monaco_fwd) {
  ...
  constexpr auto num_samples = 10000U;
  constexpr auto max_cost = 2 * 3600U;
  auto constexpr dir = direction::kForward;
...
TEST(dijkstra_ch, hamburg) {
  ...
  constexpr auto num_samples = 5000U;
  constexpr auto max_cost = 3 * 3600U;
  auto constexpr dir = direction::kForward;
...
TEST(dijkstra_ch, DISABLED_switzerland) {
  ...
  constexpr auto num_samples = 1000U;
  constexpr auto max_cost = 5 * 3600U;
  auto constexpr dir = direction::kForward;
