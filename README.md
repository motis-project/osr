...
#### Static functions
  ...
  - `resolve_endpoint(ways::routing, way_idx_t, node_idx_t, level_t, route_end, endpoint_role, bool, Fn&& f)`: resolves all profile nodes that can represent the given end of the route (`route_end`) on a matched way, either as a search root or as a search goal (`endpoint_role`). The `bool` allows returning a shared vehicle at the endpoint coordinate. `Fn f` will be called with each `node`. It's the task of the profile to give the routing algorithm an entry point to its overlay graph.
