...
#### Static functions
  ...
  - `resolve_endpoint<endpoint_role>(ways::routing, way_idx_t, node_idx_t, level_t, direction, Fn&& f)`: resolves all profile nodes that can represent a source or target on a matched way. `Fn f` will be called with each `node`. It's the task of the profile to give the routing algorithm an entry point to its overlay graph.
