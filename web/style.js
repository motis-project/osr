...
// The production MOTIS tile backend (proxied to via index.html's
// transformRequest for tile requests) also serves glyphs/sprites at its
// root, which this dev page has no local copy of.
const PROD_BASE = 'https://europe.motis-project.de/';

const LEVEL_MIN_ZOOM = 16;
...
const style = (map, level, theme) => {
    ...
    map.setStyle({
        ...
        glyphs: `${PROD_BASE}glyphs/{fontstack}/{range}.pbf`,
        // MOTIS icons (elevator, shields) from the local sprite; the `basics`
        // sprite (from VersaTiles) carries fill patterns (basics:pattern-*)
        // and markings (basics:marking-*) and isn't shipped with this repo,
        // so it's loaded from the production tile backend.
        sprite: [
            { id: 'default', url: `${PROD_BASE}/sprite` },
            { id: 'basics', url: `${PROD_BASE}sprites/basics/sprites` }
        ],
        layers: [
            {
                id: 'background',
    ...
    if (!map.hasImage(c.shield)) {
        map.addImage(
            c.shield,
            ...createShield({
                fill: theme === 'dark' ? 'hsl(0, 0%, 20%)' : 'hsl(0, 0%, 98%)',
                stroke: theme === 'dark' ? 'hsl(0, 0%, 45%)' : 'hsl(0, 0%, 75%)'
            })
        );
    }
};
...
function setStyleLayerFilters(map, level) {
    const layersWithLevelFilters = [
        ...
        {
            id: 'landuse-public-transport',
            filter: ['all', ['==', 'kind', 'public_transport'], ['any', ['!has', 'level'], ['==', 'level', level]]]
        },
        {
            id: 'indoor-corridor',
            filter: ['all', ['==', 'indoor', 'corridor'], ['==', 'level', level]]
