...
struct bidirectional_dijkstra {
  ...
  struct get_bucket { cost_t operator()(label const& l) { return l.cost(); } };
  ...
  void add_start(ways const& w, label const l, sharing_data const*) {
    if (kDebug) {
      l.get_node().print(std::cout, w); std::cout << " starting " << l.get_node().n_ << std::endl;
    }
    if (l.cost() < pq1_.n_buckets() - 1 &&
        cost1_[l.get_node().get_key()].update(l, l.get_node(), l.cost(), node::invalid())) {
      pq1_.push(l);
    }
  }
  ...
  void add_end(ways const& w, label const l, sharing_data const*) {
    if (kDebug) {
      l.get_node().print(std::cout, w); std::cout << " ending " << l.get_node().n_ << std::endl;
    }
    if (l.cost() < pq2_.n_buckets() - 1 &&
        cost2_[l.get_node().get_key()].update(l, l.get_node(), l.cost(), node::invalid())) {
      pq2_.push(l);
    }
  }
  ...
  bool run_single(typename P::parameters const& params,
    ...
    if constexpr (kDebug) {
      std::cout << "EXTRACT "; curr.print(std::cout, w); std::cout << "\n";
    }
    ...
    if (use_ch_preprocessing_ && ch_pre_ != nullptr) {
      ...
      for (auto const& neighbor : neighbors) {
        ...
        if (total >= max) {
          if (is_fwd) { max_reached_1_ = true; } else { max_reached_2_ = true; }
          continue;
        }
        if (total < max && costs[neighbor.get_key()].update(l, neighbor, static_cast<cost_t>(total), curr)) {
          auto next = label{neighbor, static_cast<cost_t>(total)};
          next.track(l, r, way_idx_t{}, neighbor.get_node(), false); // way and track info not available for shortcut
          pq.push(std::move(next));
          if constexpr (kDebug) { std::cout << " -> PUSH (CH)\n"; }
        } else {
          if constexpr (kDebug) { std::cout << " -> DOMINATED (CH)\n"; }
        }
      }
    } else {
      P::template adjacent<SearchDir, WithBlocked>(
          ...
          if constexpr (kDebug) {
            std::cout << "  NEIGHBOR "; neighbor.print(std::cout, w);
          }
          auto const total = curr_cost + edge_cost;
          if (total >= max) {
            if (is_fwd) { max_reached_1_ = true; } else { max_reached_2_ = true; }
          ...
          if (total < max &&
              costs[neighbor.get_key()].update(l, neighbor, static_cast<cost_t>(total), curr)) {
            auto next = label{neighbor, static_cast<cost_t>(total)};
            next.track(l, r, way, neighbor.get_node(), track);
            pq.push(std::move(next));
            if constexpr (kDebug) { std::cout << " -> PUSH\n"; }
          } else {
            if constexpr (kDebug) { std::cout << " -> DOMINATED\n"; }
          }
        });
    }
            ...
            if (edge_cost < kInfeasible) {
              ...
              if (opposite_it != end(*opposite_cost_map)) {
                ...
                if (opposite_curr.has_value() && opposite_curr->get_key() == curr.get_key()) {
                  ...
                  auto const opposite_pred_cost = opposite_it->second.cost(*pred);
                  auto const choose = [&](cost_t c1, cost_t c2, node m1, node m2) {
                    evaluate_meetpoint(c1, c2, is_fwd ? m1 : m2, is_fwd ? m2 : m1);
                  };
            ...
            P::template adjacent<opposite(SearchDir), WithBlocked>(
                ...
                [&](node const neighbor, std::uint32_t const, distance_t,
                  ...
                  if (!opposite_curr.has_value() ||
                      opposite_curr->get_key() != curr.get_key()) { return; }
                  auto const opposite_curr_cost = opposite_candidate->second.cost(*opposite_curr);
                  auto const pred_cost = get_cost<PathDir>(*pred);
    ...
    if (best_cost_ != kInfeasible) {
      auto const top_f = pq1_.empty() ? get_cost<direction::kForward>(meet_point_1_)
                                      : pq1_.buckets_[pq1_.get_next_bucket()].back().cost();
      auto const top_r = pq2_.empty() ? get_cost<direction::kBackward>(meet_point_2_)
                                      : pq2_.buckets_[pq2_.get_next_bucket()].back().cost();
      if (top_f + top_r >= best_cost_) { return false; }
    }
    return true;
  }
  ...
  bool run(typename P::parameters const& params,
    ...
    if (best_cost_ != kInfeasible && best_cost_ > max) { clear_mp(); return false; }
    ...
    if (use_ch_preprocessing_ && ch_pre_ != nullptr) {
      if (blocked == nullptr) {
        return dir == direction::kForward
                   ? run<direction::kForward, false>(params, w, r, max, blocked, sharing, elevations)
                   : run<direction::kBackward, false>(params, w, r, max, blocked, sharing, elevations);
      } else {
        return dir == direction::kForward
                   ? run<direction::kForward, true>(params, w, r, max, blocked, sharing, elevations)
                   : run<direction::kBackward, true>(params, w, r, max, blocked, sharing, elevations);
      }
    }
    if (blocked == nullptr) {
      return dir == direction::kForward
