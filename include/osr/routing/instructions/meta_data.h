...
struct traversed_node_hub {
  ...
  struct way_segment {
    ...
    static way_segment from(const ways& w, way_idx_t way, osm_node_idx_t from,
                            osm_node_idx_t to, bool is_way_aligned);

    way_segment flip_directions() const;

    osm_node_idx_t get_from_idx(ways const& w) const;
    osm_node_idx_t get_to_idx(ways const& w) const;

    point get_from_position(ways const& w) const;
    point get_to_position(ways const& w) const;
  ...
  static traversed_node_hub from(ways const& w, node_idx_t prev_node,
                                 ...
                                 bool is_arrive_way_aligned,
                                 bool is_exit_way_aligned,
                                 mode m);
  ...
  bool is_end_of_turn_lane(ways const& w, bool left, bool dedicated) const;

  bool does_way_names_change(ways const& w) const;
