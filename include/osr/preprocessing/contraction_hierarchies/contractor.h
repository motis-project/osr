...
struct contractor {
  ...
  static void add_loop(ways& w,
                       shortcut_storage* shortcuts,
                       node_idx_t node,
                       const bypass_path& bypass);
  /*bool has_shortcut(node_idx_t from, node_idx_t to, way_idx_t from_way,
  way_idx_t to_way, direction from_dir, direction to_dir, shortcut_storage*
  shortcut_storage) { auto const& neighbors = outgoing_neighbors_[to_idx(from)];
    return std::ranges::any_of(neighbors, [to, from_way, to_way, from_dir,
  to_dir, shortcut_storage](auto const& neighbor) { if (neighbor.node_idx == to)
  { if (shortcut_storage->is_shortcut(neighbor.way)) { auto const* sh =
  shortcut_storage->get_shortcut(neighbor.way); auto const from_resolved =
  shortcut_storage->resolve_first_way_and_dir(sh->upward_way, sh->downward_dir);
          auto const to_resolved =
  shortcut_storage->resolve_last_way_and_dir(sh->downward_way,
  sh->downward_dir); return from_resolved.way == from_way && to_resolved.way ==
  to_way && from_dir == from_resolved.dir && to_dir == to_resolved.dir;
        }
      }
      return false;
    });
  ...
  std::vector<bypass_path> find_restriction_bypasses(
      ways const& w,
      node_idx_t node,
  ...
  static std::vector<way_pos_t> get_restriction_to_ways(
    ...
    for (auto const& node_restrictions = w.r_->node_restrictions_[node];
      ...
      if (from_ == from_way_pos &&
          !shortcut_storage->is_shortcut(w.r_->node_ways_[node][to_])) {
        to_ways.push_back(to_);
      }
    }
    return to_ways;
  }
  static bool is_restriction_subset_at_node(ways const& w,
                                            node_idx_t const node,
                                            way_pos_t const way1_pos,
                                            way_pos_t const way2_pos) {
    if (!w.r_->node_is_restricted_[node]) {
      return true;
    }
    for (auto const& node_restrictions = w.r_->node_restrictions_[node];
