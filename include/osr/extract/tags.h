...
struct tags {
  explicit tags(osmium::OSMObject const& o) {
    ...
    for (auto const& t : o.tags()) {
      switch (cista::hash(std::string_view{t.key()})) {
        ...
        case cista::hash("turn:lanes"): turn_lanes_ = t.value(); break;
        case cista::hash("turn:lanes:forward"): turn_lanes_forward_ = t.value(); break;
        case cista::hash("turn:lanes:backward"): turn_lanes_backward_ = t.value(); break;
        case cista::hash("turn:lanes:both_ways"): turn_lanes_both_ways_ = t.value(); break;
  ...
  // https://wiki.openstreetmap.org/wiki/Key:turn#Indicated_turns_by_lane
  std::string_view turn_lanes_;
  std::string_view turn_lanes_forward_;
  std::string_view turn_lanes_backward_;
  std::string_view turn_lanes_both_ways_;
