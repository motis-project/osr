...
const std::filesystem::path test_cases_path = test_data_dir / "instruction-test-cases.json";
...
struct http_server::impl {
  ...
  void handle_get_traversed_node_hub(web_server::http_req_t const& req,
    ...
    try {
      ...
      std::array<bool, 2> way_directions_reverse;
      way_directions_reverse.fill(false);
      ...
      if (view.has_value()) {
        ...
        for (const auto [i, way_direction] :
             utl::enumerate<std::vector<std::string>>(
                 {"arrive_way_direction", "exit_way_direction"})) {
          const auto way_param_iter = params.find(way_direction);
          if (way_param_iter != params.end() && (*way_param_iter).has_value) {
            way_directions_reverse[i] = (*way_param_iter).value == "backward";
          }
        }
        ...
        const auto hub = traversed_node_hub::from(
            ...
            way_indices[1], nodes_indices[2], !way_directions_reverse[0],
            !way_directions_reverse[1], m);
