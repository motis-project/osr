...
export function InspectorPanel() {
  ...
  useEffect(() => {
    ...
    const hasSelectedLabel =
      ...
      selectedNodeLabels.labels.some(
        (label) => getLabelSignature(label) === selectedNodeLabelSignature,
      );

    if (!hasSelectedLabel) {
      selectNodeLabel(getLabelSignature(selectedNodeLabels.labels[0]));
    }
  }, [selectNodeLabel, selectedNodeLabelSignature, selectedNodeLabels]);

  const selectedLabel = useMemo(() => {
    if (!selectedNodeLabels || selectedNodeLabelSignature === null) {
      return null;
    }
    return (
      selectedNodeLabels.labels.find(
        (label) => getLabelSignature(label) === selectedNodeLabelSignature,
      ) ?? null
    );
  }, [selectedNodeLabelSignature, selectedNodeLabels]);

  const predecessorChain = useMemo(() => {
    if (!selectedNodeLabels || !selectedLabel || !selectedSegment) {
      return [];
    }
    return buildPredecessorChain(
      selectedNodeLabels.nodeIdx,
      selectedLabel,
      selectedSegment.nodeLabels,
    );
  }, [selectedLabel, selectedNodeLabels, selectedSegment]);
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-muted-foreground">
                    {selectedNodeLabels.labels.length} exported
                  </span>
                </div>
                <div className="space-y-1">
                  {selectedNodeLabels.labels.map((label) => {
        ...
        {activeWay && (
          <section className="space-y-2 rounded-lg border border-border/70 bg-card/70 p-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-[11px]">
              ...
              <div className="rounded border border-border/60 bg-secondary/25 p-2">
                <div className="text-muted-foreground">OSM ID</div>
                <div className="mt-1 font-mono">{activeWay.osmId ?? "-"}</div>
              </div>
              <div className="rounded border border-border/60 bg-secondary/25 p-2">
            ...
            {activeWay.properties && (
              <div className="grid grid-cols-2 gap-2 text-[11px]">
                <div className="rounded border border-border/60 bg-secondary/25 p-2">
                  <div className="text-muted-foreground">Max Speed</div>
                  <div className="mt-1 font-mono">
                    {activeWay.properties.maxSpeedKmh} km/h
                  </div>
                </div>
                <div className="rounded border border-border/60 bg-secondary/25 p-2">
                  <div className="text-muted-foreground">Destination</div>
                  <div className="mt-1 font-mono">
                    {formatBool(activeWay.properties.isDestination)}
                  </div>
                </div>
              </div>
            )}
