...
import { getPointMatchSelection } from "../lib/segment-debug";
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        {data.inputPoints.map((pt, idx) => {
          const { matches } = getPointMatchSelection(data, idx);
                  ...
                  {matches.map((m) => {
                    ...
                    return (
                      <div
                        ...
                        onMouseEnter={() => {
                          setHighlightedWay(m.wayIdx);
                          setHighlightedMatch(m.matchIdx);
                        }}
                        onMouseLeave={() => {
                      ...
                      >
                        ...
                        {m.startLabel && (
                          <div className="text-muted-foreground mt-1">
                            Penalty: {m.startLabel.matchPenalty}, Total:{" "}
                            {m.startLabel.totalStartCost}
                          </div>
                        )}
                        {(m.fwdResult || m.bwdResult) && (
                          <div className="flex gap-2 mt-1">
                            {m.fwdResult && (
                              <Badge
                                variant={
                                  m.fwdResult.reached ? "default" : "secondary"
                                }
                                className="text-[10px]"
                              >
                                fwd:{" "}
                            ...
                            {m.bwdResult && (
                              <Badge
                                variant={
                                  m.bwdResult.reached ? "default" : "secondary"
                                }
                                className="text-[10px]"
                              >
                                bwd:{" "}
