...
template boost::json::object
build_map_match_debug_json<foot<false, noop_tracking, false>>(
    ways const&,
    lookup const&,
    foot<false, noop_tracking, false>::parameters const&,
    std::vector<location> const&,
    std::vector<point_data<foot<false, noop_tracking, false>>> const&,
    std::vector<segment_data<foot<false, noop_tracking, false>>> const&,
    matched_route const&,
    std::function<geo::latlng(node_idx_t)> const&);
