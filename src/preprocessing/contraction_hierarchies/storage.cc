  ...
  for (auto& shortcut : loaded->shortcuts_) {
    if (shortcut.loop_segments.empty()) {
      ...
      shortcuts_.push_back(shortcut_data{
          shortcut.from, shortcut.to, shortcut.cost, shortcut.distance,
          shortcut.via_node, shortcut.upward_way, shortcut.upward_dir,
          shortcut.upward_distance, shortcut.upward_cost, shortcut.downward_dir,
          shortcut.downward_way, shortcut.downward_distance,
          shortcut.downward_cost, shortcut.selfloop_way_idx, loop_segments
...
void shortcut_storage::add_shortcut_to_graph(ways& w,
  ...
  auto way_nodes = w.r_->way_nodes_.add_back_sized(static_cast<uint16_t>(0U));
  way_nodes.push_back(shortcut.from);
  way_nodes.push_back(shortcut.to);

  auto way_dists =
      w.r_->way_node_dist_.add_back_sized(static_cast<uint16_t>(0U));
  way_dists.push_back(shortcut.distance);

  w.r_->way_properties_.push_back(
      way_properties{false, false, true, false, true, false, false, false, 3, 0,
                     0, 0, false, false, false, false, false, false, 3});

  // neue Way in node hinzufügen
  if (add_to_nodes) {
    w.r_->node_ways_[shortcut.from].push_back(way_idx);
    w.r_->node_in_way_idx_[shortcut.from].push_back(static_cast<uint16_t>(0U));
    w.r_->node_ways_[shortcut.to].push_back(way_idx);
    w.r_->node_in_way_idx_[shortcut.to].push_back(static_cast<uint16_t>(1U));
  }
}
...
void add_shortcut_restrictions(osr::ways& w,
  ...
  if (w.r_->node_is_restricted_[from]) {
    ...
    for (const auto& [from_, to_] : w.r_->node_restrictions_[from]) {
      ...
      if (to_ == w.r_->get_way_pos(from, shortcut.upward_way) && to_ != from_) {
  ...
  if (w.r_->node_is_restricted_[to]) {
    ...
    for (const auto& [from_, to_] : w.r_->node_restrictions_[to]) {
      if (from_ == w.r_->get_way_pos(to, shortcut.downward_way) &&
          to_ != from_) {
        w.r_->node_restrictions_[to].push_back(restriction{shortcut_pos, to_});
      }
    }
  }
}
