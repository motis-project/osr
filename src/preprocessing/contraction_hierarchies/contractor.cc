...
void contractor::calculate_neighbors(ways const& w) {
  auto const pt =
      utl::get_active_progress_tracker_or_activate("ch-contraction");
  ...
  for (node_idx_t node{0U}; node < w.n_nodes(); ++node) {
    ...
    for (auto const [way, i] : utl::zip_unchecked(
             w.r_->node_ways_[node], w.r_->node_in_way_idx_[node])) {
  ...
  for (node_idx_t node{0U};
    ...
    for (auto const outgoing_neighbours = outgoing_neighbors_[to_idx(node)];
      ...
      incoming_neighbors_[to_idx(neighbor_node_idx)].push_back(neighbor_data{
          .node_idx = node,
          .cost = neighbor_cost,
          .way = neighbor_way,
          .distance = neighbor_distance,
          .node = car::node{node, w.r_->get_way_pos(node, neighbor_way),
                            neighbor_node.dir_}});
    }
  }
}
...
std::vector<contractor::bypass_path> find_subloops(
    contractor::bypass_path const& bypass) {
...
std::vector<contractor::bypass_path> contractor::find_restriction_bypasses(
  ...
  std::vector<way_pos_t> to_restrictions =
      get_restriction_to_ways(w, node, from_way, shortcut_storage);
  ...
  dijkstra.add_start(
      w,
      {car::node{from_node, w.r_->get_way_pos(from_node, from_way), from_dir},
       0});
  dijkstra.run(car::parameters{}, w, *w.r_, 241, nullptr, nullptr, nullptr,
               direction::kForward, shortcut_storage);
  for (auto const& to : outgoing_neighbors_[to_idx(node)]) {
    ...
    if (std::ranges::find(
            to_restrictions,
            w.r_->get_way_pos(node,
                              w.r_->node_ways_[to.node_idx][to.node.way_])) !=
        to_restrictions.end()) {
      if (auto const cost = dijkstra.get_cost(to.node); cost != kInfeasible) {
        ...
        if (path.nodes.size() > 2) {
          path.total_cost =
              dijkstra.get_cost(path.nodes[path.nodes.size() - 2]) -
              dijkstra.get_cost(path.nodes[1]);

          path.nodes.erase(path.nodes.begin());
          path.nodes.pop_back();
          if (!(path.nodes[0].n_ == node &&
                path.nodes[path.nodes.size() - 1].n_ == node)) {
            continue;
          }
          ...
          for (size_t i = 0; i < path.nodes.size() - 1; ++i) {
            ...
            car::adjacent<direction::kForward, false>(
                car::parameters{}, *w.r_, current, nullptr, nullptr, nullptr,
                nullptr,
...
void contractor::add_loop(ways& w,
  ...
  way_idx_t const shortcut_way = shortcuts->add_shortcut(
      w, shortcut_data{
             ...
             .via_node = node,
             .upward_way =
                 w.r_->node_ways_[bypass.nodes[0].n_][bypass.nodes[0].way_],
             .upward_dir = bypass.nodes[0].dir_,
             .upward_distance = static_cast<distance_t>(
                 0),  // these are not needed because the path will be entirly
                      // recontructed with .loop_segments data
             .upward_cost = static_cast<cost_t>(0),
             .downward_dir = bypass.nodes[bypass.nodes.size() - 1].dir_,
             .downward_way =
                 w.r_->node_ways_[bypass.nodes[bypass.nodes.size() - 1].n_]
                                 [bypass.nodes[bypass.nodes.size() - 1].way_],
  ...
  auto const to_shortcut_pos =
      way_pos_t{static_cast<unsigned short int>(from_shortcut_pos + 1)};
  for (const auto& [from_, to_] : w.r_->node_restrictions_[node]) {
    if (to_ == bypass.nodes[0].way_ && to_ != from_) {
      w.r_->node_restrictions_[node].push_back(
          restriction{from_, from_shortcut_pos});
    }
    if (from_ == bypass.nodes[bypass.nodes.size() - 1].way_ && to_ != from_) {
      w.r_->node_restrictions_[node].push_back(
          restriction{to_shortcut_pos, to_});
    }
  }
}
void contractor::contract_node(ways& w,
  ...
  for (std::size_t i{0U}; i < incoming_neighbors_[to_idx(node)].size(); ++i) {
    ...
    if (w.r_->node_is_restricted_[node] && !shortcuts->is_shortcut(from.way)) {
      ...
      auto it = std::ranges::find_if(
          outgoing_neighbors_[to_idx(from.node_idx)],
          [&](const auto& n) { return n.node_idx == node; });
      if (it != outgoing_neighbors_[to_idx(from.node_idx)].end()) {
        for (auto bypasses =
                 find_restriction_bypasses(w_without, node, from.node_idx,
                                           from.way, from.node.dir_, shortcuts);
    ...
    for (auto const& to : existing_neighbors) {
      if (from.node_idx == to.node_idx && to.way == from.way &&
          !shortcuts->is_shortcut(to.way)) {
        continue;
      }
      if (shortcuts->shortcut_is_selfloop(from.way) ||
          shortcuts->shortcut_is_selfloop(to.way)) {
        continue;
      }
      ...
      auto is_restricted = w.r_->is_restricted(
          ...
          w.r_->get_way_pos(node, to.way), direction::kForward);
      if (is_restricted) {
        continue;
      }
    ...
    for (std::size_t j{0U}; j < outgoing_neighbors_[to_idx(node)].size(); ++j) {
      ...
      if ((is_uturn || is_restricted) && node_has_sefloop(node)) {
        ...
        for (auto const& selfloop : selfloops) {
          ...
          if (has_uturn(from.way, from.node.dir_, selfloop, direction::kForward,
                        shortcuts)) {
            continue;
          }
          if (has_uturn(selfloop, direction::kForward, to.way, to.node.dir_,
                        shortcuts)) {
            continue;
          }
          way_and_dir const selfloop_first =
              shortcuts->resolve_first_way_and_dir(selfloop,
                                                   direction::kForward);
          way_and_dir const selfloop_last = shortcuts->resolve_last_way_and_dir(
              selfloop, direction::kForward);
          auto is_restricted_from = w.r_->is_restricted(
              node, w.r_->get_way_pos(node, from.way),
              w.r_->get_way_pos(node, selfloop_first.way), direction::kForward);
          ...
          if (selfloop_data->cost > car::kUturnPenalty &&
              !is_restricted) {  // wenn der selfloop also teurer als ein uturn
                                 // wäre und dieser auch erlaubt ist
      ...
      car::resolve_all(
          *w.r_, to.node_idx, level_t{}, [&](car::node const r_node) {
            if (from.node_idx == to.node_idx &&
                from.way == w.r_->node_ways_[to.node_idx][r_node.way_] &&
                from.node.dir_ == r_node.dir_) {
              return;
            }
            if (!is_restriction_subset_at_node(w, to.node_idx, to.node.way_,
                                               r_node.way_)) {
              return;
            }
      ...
      if (min_cost != kInfeasible &&
          min_cost < kInfeasible - car::kUturnPenalty &&
          from.node_idx != to.node_idx) {
        std::vector<car::node> path = {};
        auto node_it = min_node;
        auto has_restriction = shortcuts->shortcut_has_restricted_node(
            w.r_->node_ways_[min_node.n_][min_node.way_]);
        while (true) {
          ...
          has_restriction = has_restriction ||
                            shortcuts->shortcut_has_restricted_node(pred_way) ||
                            w.r_->node_is_restricted_[pred->n_];
          node_it = *pred;
        }
        ...
        if (path.size() >= 2) {
          ...
          way_and_dir first_way_resolved = shortcuts->resolve_first_way_and_dir(
              first_way_idx, path.front().dir_);
      ...
      if (add_shortcut) {
        way_idx_t shortcut_way = shortcuts->add_shortcut(
            w, shortcut_data{.from = from.node_idx,
                             .to = to.node_idx,
                             .cost = direct_cost,
                             .distance = static_cast<unsigned short>(
        ...
        outgoing_neighbors_[to_idx(from.node_idx)].push_back(neighbor_data{
            ...
            .way = shortcut_way,
            .distance =
                static_cast<unsigned short>(from.distance + to.distance),
        ...
        if (w.r_->node_is_restricted_[from.node_idx]) {
          ...
          for (const auto& [from_, to_] :
               w.r_->node_restrictions_[from.node_idx]) {
        ...
        if (w.r_->node_is_restricted_[to.node_idx]) {
          auto shortcut_pos =
              from.node_idx != to.node_idx
                  ? w.r_->get_way_pos(to.node_idx, shortcut_way)
                  : way_pos_t{static_cast<unsigned short int>(
                        w.r_->get_way_pos(to.node_idx, shortcut_way) + 1)};
          for (const auto& [from_, to_] :
               w.r_->node_restrictions_[to.node_idx]) {
