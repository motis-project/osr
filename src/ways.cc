...
void ways::add_shortcuts() {
  ...
  auto profile_base_edge_accessible =
      [&]<typename Profile>(node_idx_t const from, node_idx_t const to,
                            way_idx_t const way, std::uint16_t const from_idx,
                            std::uint16_t const to_idx,
                            distance_t const distance) {
        auto const params = typename Profile::parameters{};
        if (Profile::node_cost(params, r_->node_properties_[from]).cost_ ==
                kInfeasible ||
            Profile::node_cost(params, r_->node_properties_[to]).cost_ ==
                kInfeasible) {
          return false;
        }
        ...
        return Profile::way_cost(params, *r_, timezones_, way, properties, dir,
                                 distance, std::nullopt, duration_t{0},
                                 direction::kForward)
                       .cost_ != kInfeasible ||
               Profile::way_cost(params, *r_, timezones_, way, properties,
                                 opposite(dir), distance, std::nullopt,
                                 duration_t{0}, direction::kForward)
                       .cost_ != kInfeasible;
      };

  auto topology_base_edge_accessible =
      [&]<typename... Profiles>(
          node_idx_t const from, node_idx_t const to, way_idx_t const way,
          std::uint16_t const from_idx, std::uint16_t const to_idx,
          distance_t const distance) {
  ...
  auto customize = [&]<bool IsBus>(auto& cch_edge_weights) {
    ...
    auto add_base_edge = [&](node_idx_t const from, node_idx_t const to,
                             way_idx_t const way, std::uint16_t const from_idx,
                             std::uint16_t const to_idx) {
      auto const distance =
      ...
      auto const way_dir =
          from_idx < to_idx ? direction::kForward : direction::kBackward;
      auto const way_cost =
          profile::way_cost(params, *r_, timezones_, way,
                            r_->way_properties_[way], way_dir, distance,
                            std::nullopt, duration_t{0}, direction::kForward)
              .cost_;
    ...
    auto combine_distance = [](distance_t const a, distance_t const b) {
      ...
      return static_cast<distance_t>(std::min(
          sum,
          static_cast<std::uint64_t>(std::numeric_limits<distance_t>::max())));
    };

    auto get_turn_cost = [&](node_idx_t const via, way_pos_t const from_way,
                             direction const from_dir, way_pos_t const to_way,
                             direction const to_dir) {
      if (r_->is_restricted<direction::kForward, IsBus>(via, from_way,
                                                        to_way)) {
        return kInfeasible;
      }
      ...
      auto cost = is_u_turn
                      ? cost_t{0U}
                      : profile::turn_cost(
                            params, r_->get_turn_angle(via, from_way, from_dir,
                                                       to_way, to_dir));
      if (is_u_turn) {
    ...
    auto customize_edge = [&](node_idx_t const u,
      ...
      for (auto const& vu : v_edge.weights_) {
        ...
        for (auto const& uw : w_edge.weights_) {
          ...
          auto const turn_cost = get_turn_cost(u, vu.to_way_, vu.to_dir_,
                                               uw.from_way_, uw.from_dir_);
      ...
      for (auto const& wu : w_edge.weights_) {
        ...
        for (auto const& uv : v_edge.weights_) {
          ...
          auto const turn_cost = get_turn_cost(u, wu.to_way_, wu.to_dir_,
                                               uv.from_way_, uv.from_dir_);
          if (turn_cost == kInfeasible) {
            continue;
          }
          changed |= relax_value(
