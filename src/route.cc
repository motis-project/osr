...
bool component_seen(ways const& w,
                    ...
                    size_t match_idx,
                    unsigned times = 1) {
  auto this_component = w.r_->way_component_[candidate_way(matches, match_idx)];
  for (auto j = 0U; j < match_idx; ++j) {
    if (w.r_->way_component_[candidate_way(matches, j)] == this_component) {
      if (--times == 0) {
        return true;
      }
    }
...
template <Profile P, typename Search, typename View>
std::optional<
    std::tuple<candidate_node, way_idx_t, typename P::node, path>>
best_candidate(typename P::parameters const& params,
  ...
  auto const n = n_candidates(m);
  for (auto j = std::size_t{0U}; j != n; ++j) {
    auto const dest_way = candidate_way(m, j);
    ...
    if (std::pow(candidate_dist_to_way(m, j), 2) >
            limit_squared_max_matching_distance &&
        j > kBottomKDefinitelyConsidered) {
      break;
    }

    for (auto const& x : {candidate_left(m, j), candidate_right(m, j)}) {
      if (x.valid()) {
        get_best(dest_way, x);
      }
    }
...
std::optional<path> route_dijkstra(
  ...
  for (auto const [i, start] : utl::enumerate(from_match)) {
    ...
    auto const c = best_candidate<P>(params, w, d, to.lvl_, to_match, max, dir,
                                     start_time, should_continue, start.way_,
                                     limit_squared_max_matching_distance);
    if (c.has_value()) {
      auto const [nc, wc, node, p] = *c;
      return reconstruct<P>(params, w, l, blocked, sharing, elevations, d, from,
                            to, start.way_, candidate_left(from_match, i),
                            candidate_right(from_match, i), wc, nc, node,
...
template <Profile P, typename FromMatch, typename ToMatch>
std::vector<std::optional<path>> route(
    ...
    FromMatch const& from_match,
    ToMatch const& to_match,
    cost_t const max,
    direction const dir,
    std::optional<routing_time_t> const start_time,
    bitvec<node_idx_t> const* blocked,
  ...
  result.resize(n_matches(to_match));
  ...
  d.reset(max);
  auto should_continue = true;
  for (auto i = std::size_t{0U}; i != n_candidates(from_match); ++i) {
    if (!should_continue && component_seen(w, from_match, i)) {
      continue;
    }
    auto const start_way = candidate_way(from_match, i);
    auto const start_left = candidate_left(from_match, i);
    auto const start_right = candidate_right(from_match, i);
    ...
    for (auto k = std::size_t{0U}; k != result.size(); ++k) {
      auto const m = match_at(to_match, k);
      auto const& t = to[k];
      auto& r = result[k];
      if (r.has_value()) {
        ++found;
