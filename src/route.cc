...
#include "osr/util/reverse.h"
  ...
  template <Profile P>
  path reconstruct_bi(
      typename P::parameters const& params, ways const& w, lookup const& l,
      bitvec<node_idx_t> const* blocked, sharing_data const* sharing,
      elevation_storage const* elevations, bidirectional<P> const& b,
      location const& from, location const& to, way_idx_t const start_way,
      candidate_node const& start_left, candidate_node const& start_right,
      way_idx_t const dest_way, candidate_node const& dest_left,
      candidate_node const& dest_right, cost_t const cost,
      direction const dir) {
    ...
    while (true) {
      ...
      if (pred.has_value()) {
        ...
        auto const expected_cost = static_cast<cost_t>(
            e.cost(forward_n) -
            b.template get_cost<direction::kForward>(*pred));
        forward_dist += add_path<P>(
            params, w, *w.r_, blocked, sharing, elevations, *pred, forward_n,
            pred_duration, {}, expected_cost,
    ...
    forward_segments.push_back(
        {.polyline_ = l.get_node_candidate_path(
             start_way, start_node_candidate.node_,
             start_node_candidate.way_dir_, false, from),
  ...
  path reconstruct(
      typename P::parameters const& params, ways const& w, lookup const& l,
      bitvec<node_idx_t> const* blocked, sharing_data const* sharing,
      elevation_storage const* elevations, Search const& search,
      location const& from, location const& to, way_idx_t const start_way,
      candidate_node const& start_left, candidate_node const& start_right,
      way_idx_t const dest_way, candidate_node const& dest_nc,
      typename P::node const dest_node, cost_t const cost, direction const dir,
  ...
  bool component_seen(ways const& w, match_view_t const& matches,
                      size_t match_idx,
<<<<<<< HEAD
                      unsigned times) {
...
>>>>>>> dev
  ...
  best_candidate(typename P::parameters const& params, ways const& w,
                 Search& search, level_t const lvl, match_view_t const& m,
                 cost_t const max, direction const dir,
                 std::optional<routing_time_t> const start_time,
                 bool should_continue, way_idx_t const start_way,
    ...
    for (auto j = std::size_t{0U}; j != n; ++j) {
      ...
      if (have_best) {
        return best_cost.cost_ < max
                   ? std::optional{std::tuple{best, dest_way, best_node,
                                              best_cost}}
                   : std::nullopt;
      }
    }
        ...
        if (auto const direct = try_direct(from_, t); direct.has_value()) {
          ...
          for (auto const start : from_match_) {
            ...
            if (c.has_value()) {
              ...
              if (do_reconstruct(p)) {
                p = reconstruct<P>(params, w, l, blocked, sharing, elevations,
                                   d_, from, t, start, *wc, *nc, n, p.cost_,
                                   dir);
  ...
  std::optional<path> route_bidirectional(
      profile_parameters const& params, ways const& w, lookup const& l,
      search_profile const profile, location const& from, location const& to,
      cost_t const max, direction const dir, double const max_match_distance,
      bitvec<node_idx_t> const* blocked, sharing_data const* sharing,
      elevation_storage const* elevations) {
  ...
  std::optional<path> route_bidirectional(
      typename P::parameters const& params, ways const& w, lookup const& l,
      bidirectional<P>& b, location const& from, location const& to,
      match_view_t const& from_match, match_view_t const& to_match,
      cost_t const max, direction const dir, bitvec<node_idx_t> const* blocked,
    ...
    for (auto i = std::size_t{0U}; i != from_match.size(); ++i) {
      ...
      for (auto const* nc : {&start_left, &start_right}) {
        if (nc->valid() && nc->cost_ < max) {
          auto const start_cost = P::way_cost(
              ...
              static_cast<distance_t>(nc->dist_to_node_), {}, duration_t{0},
              dir);
      ...
      for (auto j = std::size_t{0U}; j != to_match.size(); ++j) {
        ...
        return reconstruct_bi(params, w, l, blocked, sharing, elevations, b,
                              from, to, start_way, start_left, start_right,
                              end_way, end_left, end_right, cost, dir);
      }
      b.pq1_.clear();
      b.pq2_.clear();
      b.cost2_.clear();
  ...
  std::optional<path> route_dijkstra(
      typename P::parameters const& params, ways const& w, lookup const& l,
      dijkstra<P>& d, location const& from, location const& to,
      match_view_t const& from_match, match_view_t const& to_match,
      cost_t const max, direction const dir,
      std::optional<routing_time_t> const start_time,
      bitvec<node_idx_t> const* blocked, sharing_data const* sharing,
    ...
    for (auto i = std::size_t{0U}; i != from_match.size(); ++i) {
      ...
      for (auto const* nc : {&start_left, &start_right}) {
        if (nc->valid() && nc->cost_ < max) {
          ...
          P::resolve_start_node(*w.r_, start_way, nc->node_, from.lvl_, dir,
                                [&](auto const node) {
      ...
      if (c.has_value()) {
        ...
        return reconstruct<P>(params, w, l, blocked, sharing, elevations, d,
                              from, to, start_way, start_left, start_right, wc,
                              nc, node, p.cost_, dir, start_time);
      }
    }
  ...
  std::optional<path> route_astar(
      typename P::parameters const& params, ways const& w, lookup const& l,
      astar<P>& a, location const& from, location const& to,
      match_view_t const& from_match, match_view_t const& to_match,
      cost_t const max, direction const dir,
      std::optional<routing_time_t> const start_time,
      bitvec<node_idx_t> const* blocked, sharing_data const* sharing,
    ...
    for (auto i = std::size_t{0U}; i != from_match.size(); ++i) {
      ...
      auto const c = best_candidate<P>(
          params, w, a, to.lvl_, to_match, max, dir, start_time,
          should_continue, start_way, limit_squared_max_matching_distance);
      if (c.has_value()) {
        auto const [nc, wc, node, p] = *c;
        return reconstruct<P>(params, w, l, blocked, sharing, elevations, a,
                              from, to, start_way, start_left, start_right, wc,
                              nc, node, p.cost_, dir, start_time);
      }
    }
  ...
  std::vector<std::optional<path>> route(
      typename P::parameters const& params, ways const& w, lookup const& l,
      dijkstra<P>& d, location const& from, std::vector<location> const& to,
      match_view_t const& from_match, match_result const& to_match,
      cost_t const max, direction const dir,
      std::optional<routing_time_t> const start_time,
      bitvec<node_idx_t> const* blocked, sharing_data const* sharing,
      elevation_storage const* elevations,
          ...
          if (c.has_value()) {
            ...
            if (do_reconstruct(p)) {
              p = reconstruct<P>(params, w, l, blocked, sharing, elevations, d,
                                 ...
                                 wc, nc, n, p.cost_, dir, start_time);
              p.uses_elevator_ = true;
            }
  ...
  std::vector<std::optional<path>> route(
      profile_parameters const& params, ways const& w, lookup const& l,
      search_profile const profile, location const& from,
      std::vector<location> const& to, cost_t const max, direction const dir,
      double const max_match_distance, bitvec<node_idx_t> const* blocked,
      sharing_data const* sharing, elevation_storage const* elevations,
      std::function<bool(path const&)> const& do_reconstruct,
      std::optional<routing_time_t> const start_time) {
    return with_profile(
  ...
  std::vector<std::optional<path>> route(
      profile_parameters const& params, ways const& w, lookup const& l,
      search_profile const profile, location const& from,
      std::vector<location> const& to, match_view_t const& from_match,
      match_result const& to_match, cost_t const max, direction const dir,
      bitvec<node_idx_t> const* blocked, sharing_data const* sharing,
      elevation_storage const* elevations,
      std::function<bool(path const&)> const& do_reconstruct,
      std::optional<routing_time_t> const start_time) {
  ...
  std::optional<path> route(
      ...
      cost_t const max, direction const dir, bitvec<node_idx_t> const* blocked,
      sharing_data const* sharing, elevation_storage const* elevations,
      routing_algorithm algo, std::optional<routing_time_t> const start_time) {
    if (from_match.empty() || to_match.empty()) {
      return std::nullopt;
    }
      ...
      case routing_algorithm::kDijkstra:
        return with_profile(profile, [&]<Profile P>(P&&) {
          return route_dijkstra(std::get<typename P::parameters>(params), w, l,
                                get_dijkstra<P>(), from, to, from_match,
                                to_match, max, dir, start_time, blocked,
                                sharing, elevations);
        });
  ...
  std::optional<path> route(
      ...
      cost_t const max, direction const dir, double const max_match_distance,
      bitvec<node_idx_t> const* blocked, sharing_data const* sharing,
      elevation_storage const* elevations, routing_algorithm algo,
      std::optional<routing_time_t> const start_time) {
    if (profile == search_profile::kBikeSharing ||
        profile == search_profile::kCarSharing ||
