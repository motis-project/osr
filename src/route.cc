...
template <Profile P>
std::optional<path> route_bidirectional_dijkstra(typename P::parameters const& params,
                                                 ways const& w,
                                                 lookup const& l,
                                                 bidirectional_dijkstra<P>& bd,
                                                 location const& from,
                                                 location const& to,
                                                 match_view_t from_match,
                                                 match_view_t to_match,
  ...
  for (auto const [i, start] : utl::enumerate(from_match)) {
    ...
    for (auto const [j, end] : utl::enumerate(to_match)) {
          ...
          P::resolve_start_node(
              *w.r_, end_way, nc->node_, to.lvl_, opposite(dir),
              [&](auto const node) {
                auto label = typename P::label{node, nc->cost_};
                label.track(label, *w.r_, end_way, node.get_node(), false);
                bd.add_end(w, label, sharing);
              });
        }
      }

      if (bd.pq1_.empty() || bd.pq2_.empty()) {
        continue;
      }

      auto const should_continue =
          bd.run(params, w, *w.r_, max, blocked, sharing, elevations, dir);

      if (bd.meet_point_1_.get_node() == node_idx_t::invalid()) {
        if (should_continue) {
          continue;
        }
        return std::nullopt;
      ...
      auto const cost = bd.get_cost_to_mp(bd.meet_point_1_, bd.meet_point_2_);

      return reconstruct_ch_preprocessing(params, w, l, blocked, sharing, elevations,
                                        bd, from, to, start, end, cost, dir);
    }
    bd.pq1_.clear();
    bd.pq2_.clear();
    bd.cost1_.clear();
...
std::optional<path> route_bidirectional_dijkstra(profile_parameters const& params,
                                                 ways const& w,
                                                 lookup const& l,
                                                 search_profile const profile,
                                                 location const& from,
  ...
  return with_profile(profile, [&]<Profile P>(P&&) -> std::optional<path> {
    ...
    return route_bidirectional_dijkstra(pp, w, l, get_bidirectional_dijkstra<P>(),
...
std::optional<path> route(profile_parameters const& params,
  ...
  switch (algo) {
    ...
    case routing_algorithm::kBiDijkstra:
      return with_profile(profile, [&]<Profile P>(P&&) {
        return route_bidirectional_dijkstra(std::get<typename P::parameters>(params), w,
                                           l, get_bidirectional_dijkstra<P>(), from, to,
                                           from_match, to_match, max, dir, blocked,
                                           sharing, elevations);
      });
  }
  ...
  switch (algo) {
    ...
    case routing_algorithm::kAStarBi:
      return route_bidirectional(params, w, l, profile, from, to, max, dir,
                                 ...
                                 elevations);
    case routing_algorithm::kBiDijkstra:
      return route_bidirectional_dijkstra(params, w, l, profile, from, to, max, dir,
                                         max_match_distance, blocked, sharing, elevations);
  }
  throw utl::fail("not implemented");
}
